...
@onready var box = PhysicsServer3D.box_shape_create()
...
func spawn_cubes() -> void:
	...
	var cube : InnerCube = InnerCube.new(PhysicsServer3D.body_create(), mm.visible_instance_count)
	PhysicsServer3D.body_set_mode(cube, PhysicsServer3D.BODY_MODE_RIGID)
	PhysicsServer3D.body_add_shape(cube.rID,box)
	PhysicsServer3D.body_set_space(cube.rID,get_world_3d().space)

	mm.set_instance_transform(cube.cID,spawn_transform)
	PhysicsServer3D.body_set_state(cube.rID,PhysicsServer3D.BODY_STATE_TRANSFORM,spawn_transform)
	PhysicsServer3D.body_apply_central_impulse(cube.rID,Vector3(randf_range(-5,5),-45,randf_range(-5,5)))
